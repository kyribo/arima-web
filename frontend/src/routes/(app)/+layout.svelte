...
<div class="min-h-screen bg-gray-50 dark:bg-neutral-900 flex transition-colors duration-300">
	...
	>
		...
		>
			<div class="w-8 h-8 rounded-lg bg-gradient-to-br from-blue-500 to-purple-500 shrink-0"></div>
			{#if !isSidebarCollapsed}
				<span
				...
				{@const isActive =
					...
					(hasChildren &&
						(item.children!.some((c) => $page.url.pathname.startsWith(c.href)) ||
							item.children!.some(
								(c) => c.children && c.children.some((gc) => $page.url.pathname.startsWith(gc.href))
							)))}
				...
				<div>
					...
					{:else}
						...
						>
							...
							{/if}
							{#if !isSidebarCollapsed}
								<span class="{item.icon ? 'ml-3' : ''} truncate" transition:fade={{ duration: 100 }}
									>{item.name}</span
								>
							{/if}
						</a>
