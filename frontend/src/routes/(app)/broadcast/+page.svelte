<script lang="ts">
    ...
    let channels = $state(['In-App']); // Default to In-App
    let isSending = $state(false);
    let showSuccess = $state(false);

    // Mock History
    let history = $state([
        { id: 1, title: 'System Maintenance', target: 'All Users', date: '2023-10-25 14:00', status: 'Sent' },
        { id: 2, title: 'New Feature Alert', target: 'Editors', date: '2023-10-20 09:30', status: 'Sent' },
    ]);
	...
	<div class="flex justify-between items-start">
		<div>
			...
			<p class="text-gray-500 dark:text-gray-400 mt-1">
				Send announcements to your users via multiple channels.
			</p>
		</div>
	</div>
