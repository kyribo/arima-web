export interface MenuItem {
    name: string;
    href: string;
    icon?: string;
    hidden?: boolean;
    children?: MenuItem[];
}

export const menuItems: MenuItem[] = [
    {
        name: 'Dashboard',
        href: '/dashboard',
        icon: 'M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6'
    },
    {
        name: 'Trans Data',
        href: '#',
        hidden: true,
        icon: 'M8 7h12m0 0l-4-4m4 4l-4 4m0 6H4m0 0l4 4m-4-4l4-4',
        children: [
            {
                name: 'Analytics',
                href: '/analytics',
                icon: 'M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 002 2h2a2 2 0 002-2z'
            },
            {
                name: 'Monitoring',
                href: '#',
                icon: 'M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 002 2h2a2 2 0 002-2z',
                children: [
                    { name: 'Basic Limit', href: '/monitoring/basic-limit' },
                    { name: 'Loan Limit', href: '/monitoring/loan-limit' }
                ]
            },
            {
                name: 'Broadcast',
                href: '#',
                icon: 'M11 5.882V19.24a1.76 1.76 0 01-3.417.592l-2.147-6.15M18 13a3 3 0 100-6M5.436 13.683A4.001 4.001 0 017 6h1.832c4.1 0 7.625-1.234 9.168-3v14c-1.543-1.766-5.067-3-9.168-3H7a3.988 3.988 0 01-1.564-.317z',
                children: [
                    { name: 'Statement', href: '/broadcast/statement' },
                    { name: 'Stock Info', href: '/broadcast/stock-info' },
                    { name: 'Other', href: '/broadcast/other' }
                ]
            },
            { name: 'Stock Information', href: '/stock-information', icon: 'M13 7h8m0 0v8m0-8l-8 8-4-4-6 6' },
            {
                name: 'Weekly Report PTB',
                href: '/weekly-report-ptb',
                icon: 'M9 17v-2m3 2v-4m3 4v-6m2 10H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z'
            },
            {
                name: 'Capping Stock',
                href: '#',
                icon: 'M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4',
                children: [
                    { name: 'History', href: '/capping-stock/history' },
                    { name: 'Review', href: '/capping-stock/review' }
                ]
            },
            { name: 'Reports', href: '/reports', icon: 'M9 17v-2m3 2v-4m3 4v-6m2 10H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z' },
            { name: 'Status', href: '/status', icon: 'M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z' },
            { name: 'Users', href: '/users', icon: 'M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z' },
            {
                name: 'Settings',
                href: '#',
                icon: 'M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z',
                children: [
                    { name: 'Profile', href: '/settings/profile' },
                    { name: 'Security', href: '/settings/security' },
                    { name: 'Notifications', href: '/settings/notification' }
                ]
            },
        ]
    },
    { name: 'Risk Event', href: '/risk-event', icon: 'M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z' },
    {
        name: 'Upload Data',
        href: '#',
        icon: 'M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12',
        children: [
            { name: 'Due Date', href: '/upload-data/due-date' },
            { name: 'Basic Limit', href: '/upload-data/basic-limit', hidden: true },
            { name: 'Reguler Limit', href: '/upload-data/reguler-limit', hidden: true }
        ]
    },
];
